...
class LoginController: UIViewController {
    ...
    private let registerButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Sign Up :D", for: .normal)
        button.setTitleColor(.twitterBlue, for: .normal)
        button.backgroundColor = .white
        button.heightAnchor.constraint(lessThanOrEqualToConstant: 50)
        button.layer.cornerRadius = 5
    ...
    func configureUI() {
        ...
        let stack = UIStackView(arrangedSubviews: [emailContainerView, passwordContainerView, loginButton, registerButton])
        stack.axis = .vertical
        stack.spacing = 20
    ...
    @objc func handleLogin() {
        ...
        AuthService.shared.logUserIn(email: email, password: password) { (result, err) in
            if let err = err {
                print("DEBUG: Error logging user in")
                return
            }
